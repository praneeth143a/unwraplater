...
const Helpers = (() => {
     ...
     * @param {HTMLElement} container - Container to put QR code in
     * @param {Object} options - Optional configurations like size, color
     * @returns {HTMLCanvasElement} The generated QR code canvas
     */
    const generateQRCode = (text, container, options = {}) => {
        ...
        const settings = {
            ...
            correctLevel: 'H' // Highest error correction level
        };
        
        // Create QR code using ASCII characters
        const qrCanvas = document.createElement('canvas');
        const ctx = qrCanvas.getContext('2d');
        const cellSize = Math.floor(settings.size / 25); // QR code is typically 25x25 cells
        ...
        // Generate binary pattern for QR (simplified version - not actual QR encoding)
        // In a real implementation, this would use proper QR code encoding algorithms
        ...
        for (let y = 0; y < pattern.length; y++) {
            for (let x = 0; x < pattern[y].length; x++) {
                if (pattern[y][x] === 1) {
                    ctx.fillRect(x * cellSize + cellSize, y * cellSize + cellSize, cellSize, cellSize);
                }
            }
        }
        
        // Draw position markers (three corners)
        drawPositionMarker(ctx, cellSize, cellSize, cellSize * 7);
        drawPositionMarker(ctx, cellSize, cellSize * 17, cellSize * 7);
        drawPositionMarker(ctx, cellSize * 17, cellSize, cellSize * 7);
    ...
    /**
     * Helper function to draw position markers for QR code
     * @private
     */
    const drawPositionMarker = (ctx, x, y, size) => {
        const outerSize = size;
        const innerSize = Math.floor(size * 0.7);
        const dotSize = Math.floor(size * 0.3);
     ...
     * This is NOT an actual QR code encoding - just a visual demonstration
     * @private
     */
    const generateQRPattern = (text) => {
        // Hash the text to generate a pseudo-random pattern
        let hash = 0;
        ...
        // Create a 23x23 matrix (25x25 with position markers)
        const pattern = Array(23).fill().map(() => Array(23).fill(0));
        
        // Seed the pattern based on the hash
        const random = seedRandom(hash);
        
        // Fill with pseudo-random data based on the seed
        for (let y = 0; y < 23; y++) {
            for (let x = 0; x < 23; x++) {
                // Skip positions where the markers will be
                if ((x < 7 && y < 7) || (x > 15 && y < 7) || (x < 7 && y > 15)) {
                    continue;
                }
